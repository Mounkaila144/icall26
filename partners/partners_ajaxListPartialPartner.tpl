...
<h3>{__('Installers')}</h3>    
<div>
  <a href="#" class="btn" id="Partner-New" title="{__('New installer')}" ><i class="fa fa-plus" style="margin-right:10px;"></i>{*<img  src="{url('/icons/new.gif','picture')}" alt="{__('new')}"/>*}{__('New installer')}</a>   

</div>
{include file="./_pagers/default/_default_pager_top.tpl" pager=$pager formfilter=$formFilter class="Partner"}
<div>
        <button style="width:135px" id="Partner-filter" class="btn-table">{__("Filter")}</button> 
<button style="width:135px" id="Partner-init" class="btn-table">{__("Init")}</button>
    </div>
        ...
        <td class="Partner-count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>
          {*  {if $pager->getNbItems()>5}
            ...
            <td>                
               ...
               <td>                
                 <a href="#" class="Partner-ChangeIsActive" name="{$item->get('id')}" id="{$item->get('is_active')}"><img src="{url('/icons/','picture')}{$item->get('is_active')}.gif" alt='{__($item->get("is_active"))}' title='{__($item->get("is_active"))}'/></a>                           
            </td> 
            <td>               
...
<script type="text/javascript">
    ...
    function changePartnerIsActiveState(resp) 
        {
            $.each(resp.selection?resp.selection:[resp.id], function () {              
                sel="a.Partner-ChangeIsActive[name="+this+"]";
        ...
        function updateSitePager(n)
        {
         ...
         $(".Partner").dblclick( function () {                    
                return $.ajax2({ data : { Partner : $(this).attr('id').replace('Partner-','') },
                                loading: "#tab-site-dashboard-x-settings-loading",
                                url:"{url_to('partners_ajax',['action'=>'ViewPartner'])}",
                                errorTarget: ".site-errors",
                                target: "#actions"});
          ...
          $(".Partner-Delete").click( function () { 
                if (!confirm('{__("Partner \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ Partner: $(this).attr('id') },
           ...
           $(".Partner-ChangeIsActive").click( function () { 
                return $.ajax2({ data: { Partner : { id: this.name, value: this.id, token: '{mfForm::getToken("ChangeForm")}' } },
                                 url: "{url_to('partners_ajax',['action'=>'ChangeIsActivePartner'])}",
                                 errorTarget: ".site-errors",
                                 loading: "#tab-site-dashboard-x-settings-loading",
                                 success: function(resp) {
                                               if (resp.action=='ChangeIsActivePartner')
                                                    changePartnerIsActiveState(resp);
                                          }
                });    
            });
	{* $('.footable').footable(); *}
