...
class SystemVersionsPagerViewCsvFile implements ArrayAccess ,Iterator, Countable {
    ...
    function __construct($import_file,$nb_items_by_page=50)
    {
        ...
        $this->file->setSchema(array_merge($import_file->getSchema(),array("import status"=>"import status","mode"=>"mode")));
        $this->file->open();
    ...
    {
        $this->seek_pointer = ($position<0)?0:$position;
    }
    
    function setErrorField($field)
    {
        $this->error_field = ($field==null)?"":$field;
    }
    
    function isErrorField($field)
    {
        return ($this->error_field==$field);
    }
    ...
    {   
        ...
        {            
            ...
            $this->file->seek($this->seek_pointer);
