 ...
 /*   
  ...
  static function loadContractWithCustomerAndPartnerFromSelection($selection,$site=null)
    ...
    {     
      ...
      static function getCampaignsForSelect(ConditionsQuery $where,$user,$site=null)
    {     
       /*   $cache= new mfCacheFile('campaigns.contract.conditions.'.md5($where->getWhere(),$user->hasCredential(array(array('contract_filter_equal_campaign_active')))),'admin',$site);
        if ($cache->isCached())       
            return unserialize($cache->read());   
*/
        $db=mfSiteDatabase::getInstance();
                $db->setParameters($where->getParameters())               
                ->setQuery("SELECT ".CustomerMeetingCampaign::getFieldsAndKeyWithTable()." FROM ".CustomerContract::getTable().
                           " LEFT JOIN ".CustomerContract::getOuterForJoin('campaign_id').                            
                           " WHERE ".CustomerMeetingCampaign::getTableField('id')." IS NOT NULL ".
                           " GROUP BY ".CustomerMeetingCampaign::getTableField('id').
                           " ORDER BY ". CustomerMeetingCampaign::getTableField('name')." ASC ".                          
                           ";")               
                ->makeSiteSqlQuery($site); 
        // echo $db->getQuery();
        if (!$db->getNumRows())
    ...
    {        
     ...
     static function getCampaigns(ConditionsQuery $where,$user,$site=null)
    {        
        $cache= new mfCacheFile('campaigns.contract.conditions.select2.'.md5($where->getWhere()),'admin',$site);
        if ($cache->isCached())       
            return unserialize($cache->read()); 
        $db=mfSiteDatabase::getInstance();
                $db->setParameters($where->getParameters())               
   ...
   static function createDefaultProductForContracts(Product $product)
     ...
     static function getCampaignsFromPager($pager)
    {
       ...
       $db=mfSiteDatabase::getInstance();
                $db->setParameters(array())
                ->setObjects(array('CustomerContractCompany'))
                ->setQuery("SELECT {fields},".CustomerContract::getTableField('id')." as contract_id FROM ". CustomerMeetingCampaign::getTable().
                           " LEFT JOIN ".CustomerContract::getInnerForJoin('campaign_id').                         
                           " WHERE ".CustomerContract::getTableField('id')."  IN(".implode(",",array_keys($pager->getItems())).")".
                           ";")               
                ->makeSiteSqlQuery($pager->getSite()); 
       //  echo $db->getQuery();
        ...
        {                   
           if (!isset($pager[$items->get('contract_id')]))
               continue;           
           $pager[$items->get('contract_id')]->set('campaign_id',$items->hasCustomerMeetingCampaign()?$items->getCustomerMeetingCampaign():false);
        }        
    }
