...
class CustomerContractStatusI18nBase extends mfObjectI18n {
     
    protected static $fields=array('value','status_id','lang','created_at','updated_at');
    const table="t_customers_contracts_status_i18n"; 
    protected static $foreignKeys=array('status_id'=>'CustomerContractStatus'); // By default
      ...
      function delete()
    {
        if (parent::delete()===false)       
            return $this;
        if (!$this->hasSibbling())
            $this->getCustomerContractStatus()->delete();
        return $this;
    }  
   
     function getCustomerContractStatus()
    {
       if (!$this->_status_id)
       {
          $this->_status_id=new CustomerContractStatus($this->get('status_id'),$this->getSite());          
       }   
