...
require_once dirname(__FILE__)."/Forms/ContractStatusImportForm.class.php";
require_once dirname(__FILE__)."/Forms/ContractStatusImportFileForm.class.php";

class ContractStatusImport extends ImportCore {
              
    function __construct($file, $type, $site = null) {
        parent::__construct($file, $type, $site);
        if ($type=='zip')
        {    
            $this->loader=new ImportZip('ContractStatus',$site);
    ...
    {                  
        try
        {     
           $this->setForm(new ContractStatusImportForm($this->getSourcePathForFiles(),$this->getSite()));  
           $this->verifyHeader();
           $this->initialize();          
           ...
           {                                                      
                ...
                {      
                    ...
                    $status_i18n->getCustomerContractStatus()->save();
                    $status_i18n->set('status_id',$status_i18n->getCustomerContractStatus());
                    $status_i18n->save();    
                    if ($icon)
                    {
