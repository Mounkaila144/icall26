...
class CustomerContractsPager extends Pager {
    ...
    {              
       ...
       {                       
              ...
              {
                  $item->set('state_id',null);
              }                  
              $item->set('telepro_id',$items->hasTelepro()?$items->getTelepro():null);             
              $item->set('financial_partner_id',$items->hasPartner()?$items->getPartner():null);              
    ...
    function getTurnover()
    {
        if (!$this->isExecuted())
            return null;
       return CustomerContractUtils::getTurnoverFromPager(array_keys($this->items),$this->getSite());
    }
