{messages class="{$site->getSiteID()}-dialog-customers-contract-site-errors"}
{include file="./_pagers/default/_default_pager_top.tpl" pager=$pager formfilter=$formFilter class="`$site->getSiteID()`-Dialog-CustomerContracts"}
  ...
  <tr class="list-header">
   ...
   <tr class="input-list">
       ...
       <td>{* date *}       
        <div>
               {__('from')}
                <input class="Dialog-CustomerContracts range" id="dialog_opened_at_from" type="text" size="10" name="opened_at[from]" value="{format_date((string)$formFilter.range.opened_at.from,'a')}"/>
            </div>
            <div>
               {__('to')}
                <input  class="Dialog-CustomerContracts range" id="dialog_opened_at_to" type="text" size="10" name="opened_at[to]" value="{format_date((string)$formFilter.range.opened_at.to,'a')}"/>
            </div>                  
       </td>
       <td>{* customer *}
         ...
         <td class="Dialog-CustomerContracts-count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>
            {if $pager->getNbItems()>5}
                <td>        
                    ...
                    <input class="{$site->getSiteID()}-Dialog-CustomerContractsStatus-selection" type="checkbox" id="{$item->getCustomerContractStatusI18n()->get('id')}" name="{$item->getCustomerContractStatusI18n()->get('name')}"/>   
                    {/if}
                </td>
            {/if} 
         </td>          
...
{include file="./_pagers/default/_default_pager_bottom.tpl" pager=$pager class="`$site->getSiteID()`-Dialog-CustomerContracts"}


<script type="text/javascript">
    
      var dates = $( ".Dialog-CustomerContracts#dialog_opened_at_from, .Dialog-CustomerContracts#dialog_opened_at_to" ).datepicker({
			onSelect: function( selectedDate ) {
				var option = this.id == "dialog_opened_at_from" ? "minDate" : "maxDate",
     ...
     $("#{$site->getSiteID()}-dialogListFilterContracts").data('pager',[{foreach $pager as $item} { opened_at: "{format_date($item->get('opened_at'),'p')}" ,customer: "{$item->getCustomer()->getLastname()} {$item->getCustomer()->getFirstname()}" }, {/foreach}]);   
        
     $("#{$site->getSiteID()}-dialogListFilterContracts").data('selected',"{(string)$formFilter.selected}");  
    ...
    function getDialogContractsFilterParameters()
        ...
        function updateDialogContractsFilter()
        {
           return $.ajax2({ data: getDialogContractsFilterParameters(), 
                            ...
                            errorTarget: ".{$site->getSiteID()}-dialog-customers-contract-site-errors",
                            loading: "#tab-site-{$site->getSiteID()}-dashboard-site-customers-contract-loading",
                            target: "#{$site->getSiteID()}-dialogListFilterContracts" });
        }
        
         function updateSiteDialogPager(n)
        {
           page_active=$(".{$site->getSiteID()}-Dialog-CustomerContracts-pager .Dialog-CustomerContracts-active").html()?parseInt($(".{$site->getSiteID()}-Dialog-CustomerContracts-pager .Dialog-CustomerContracts-active").html()):1;
           records_by_page=$("[name={$site->getSiteID()}-CustomerContracts-nbitemsbypage]").val();
           start=(records_by_page!="*")?(page_active-1)*records_by_page+1:1;
           $(".{$site->getSiteID()}-Dialog-CustomerContracts-count").each(function(id) { $(this).html(start+id) }); // Update index column           
           nb_results=parseInt($("#{$site->getSiteID()}-CustomerContracts-nb_results").html())-n;
           $("#{$site->getSiteID()}-Dialog-CustomerContracts-nb_results").html((nb_results>1?nb_results+" {__('results')}":"{__('one result')}"));
           $("#{$site->getSiteID()}-Dialog-CustomerContracts-end_result").html($(".{$site->getSiteID()}-Dialog-CustomerContracts-count:last").html());
        }
        ...
        $(".{$site->getSiteID()}-Dialog-CustomerContracts-pager").click(function () {             
                return $.ajax2({ data: getDialogContractsFilterParameters(), 
        ...
        $(".Dialog-CustomerContracts.list").click(function () { 
            ...
            $("#{$site->getSiteID()}-dialogListFilterContracts").data('selected',$(this).attr('id'));
            index=$(".Dialog-CustomerContracts.list.selected").index()-2;  {* 2 : number of <tr> before *}            
            $("#{$site->getSiteID()}-dialogListFilterContracts").data('object',$("#{$site->getSiteID()}-dialogListFilterContracts").data('pager')[index]);
        });
</script>    