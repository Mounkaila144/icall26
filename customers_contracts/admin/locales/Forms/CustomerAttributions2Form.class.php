    ...
    {            
       ...
       {    
            ...
            if ($settings->hasAssistant() && $this->getUser()->hasCredential(array(array('superadmin','admin','contract_attributions_modify_assistant'))))
                $contributors['assistant']='ASSISTANT';  
            if ($this->getUser()->hasCredential(array(array('superadmin','admin','contract_attributions_modify_team'))))
       ...
       {                         
           ...
           $this->embedForm($name, new UserContributor($this->getDefault($name)));
           if ($name=='team')
               $this->$name->addValidator('team_id',new mfValidatorChoice(array('key'=>true,'required'=>false,'choices'=>array(""=>"")+UserTeamUtils::getFieldValuesForSelect('name'))));
           else
...
class CustomerAttributions2Form extends mfForm {
