...
class CustomerContractsFormFilter extends mfFormFilterBase {
    ...
    {                 
      ...
      // elseif ($this->getUser()->hasGroups('sales_manager'))
       ...
       //    $polluter_query=" LEFT JOIN ".CustomerContract::getOuterForJoin('polluter_id');
       ...
       {                    
         ...
         //     $this->getColumns()->set('opc_range','0005_column-opc_range');
       } 
       ...
       mfContext::getInstance()->getEventManager()->notify(new mfEvent($this, 'contract.filter.config'));   
    ...
    function updateFilterColumns()
    {
       ...
       if (($cols=$this->getUser()->getStorage()->read('contract_cols')) && !$this->getDefault('cols'))
       {                           
         $this->setDefault('cols',$cols);
         $this->setDefault('sizes',$this->getUser()->getStorage()->read('contract_sizes'));         
       }   
       elseif (($properties=UserProperty::load('list_contract',$this->getUser()->getGuardUser())) && !$this->getDefault('cols'))
       {           
       ...
       {                      
           UserProperty::register('list_contract',$this->getUser()->getGuardUser(),$this->getDefaultColumns()->toArray(),$this['sizes']->getValue());           
    ...
    function __getParameters($values)    
    {
     ...
     {                    
        $this->getUser()->getStorage()->write('contract_cols',$this['cols']->getValue());         
        $this->getUser()->getStorage()->write('contract_sizes',$this['sizes']->getValue());  
        UserProperty::register('list_contract',$this->getUser()->getGuardUser(),$this['cols']->getValue(),$this['sizes']->getValue()); 
        return $this;
     }
