...
<h3>{__("View doc model for polluter [%s]",$item_i18n->getModel()->getPolluter()->get('name'))}</h3>
<div>
...
<table class="tab-form">   
    ...
    {if $user->hasCredential([['superadmin']])}
     <tr class="full-with">
        ...
        <td>
            <textarea disabled="" readonly="" cols="80" rows="10">{$item_i18n->getVariables()->implode("\n")}</textarea>
        </td>
    </tr>
    {/if}        
     <tr class="full-with">
        <td class="label"><span>{__("Mapping")}</span></td>
        <td>
            <textarea  class="PartnerPolluterModelI18n"  name="mapping" cols="80" rows="10">{$item_i18n->get('mapping')}</textarea>
        </td>
    </tr>        
</table>
      ...
      $('#PartnerPolluterModel-Save').click(function(){                             
          ...
          return $.ajax2({ data : params,  
                           ...
                           errorTarget: ".PartnerPolluterModel-errors",
                            loading: "#tab-site-dashboard-x-settings-loading",  
                           url: "{url_to('partners_polluter_ajax',['action'=>'SaveDocModelI18nForPolluter'])}",
                           target: "#actions" }); 
        });  
