...
{if $product->isLoaded()}
    <div id="MasterAction">
        {component name="/products_items/listMasterItems" product=$product} 
    </div>
    <div id="SlaveAction">
        {component name="/products_items/listSlaveItems" product=$product}  
    </div>
    <div id="ItemsAction">
        {component name="/products_items/listItems" product=$product}  
    </div>
        ...
        $(".Product-ViewItemsMasterSlaveList").click( function () {                    
                return $.ajax2({ data : { ProductItem : $(this).attr('id')},
                                loading: "#tab-site-dashboard-x-settings-loading",
                                url:"{url_to('products_items_ajax',['action'=>'ViewItemListMasterSlaveForProduct'])}",
                                errorTarget: ".site-errors",
                                target: "#actions"});
         });
        </script>
