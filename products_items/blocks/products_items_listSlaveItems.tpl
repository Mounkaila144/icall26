<div>
<h4>{__('List For Slave')}</h4>  
{include file="./_pagers/default/_default_pager_top.tpl" pager=$pager formFilter=$formFilter class="ProductSlave"}
<button id="ProductSlave-filter" class="btn-table">{__("Filter")}</button> 
<button id="ProductSlave-init" class="btn-table">{__("Init")}</button>
 ...
 <table cellpadding="0" cellspacing="0" class="tabl-list footable table">  
    ...
    {* search/equal/range *}
     <tr class="input-list">
       <td>{* # *}</td>   
       <td>{* id *}
           <input type="text" class="ProductSlave-search" name="id" value="{$formFilter.search.id}"/>
       </td> 
       <td>{* linked id *}</td>
       {if $user->hasCredential([['superadmin','settings_product_item_mark_list']])}
          <td></td>
      ...
      {/if} 
       {if $user->hasCredential([['superadmin','settings_product_item_unit_list']])}
        ...
        {/if}
           {if $user->hasCredential([['superadmin','settings_product_item_is_active_list']])}
         <td>{* is_active *}
         {html_options class="ProductSlave-equal" name="is_active" options=$formFilter->equal.is_active->getOption('choices') selected=(string)$formFilter.equal.is_active}
        </td> 
        {/if} 
       <td>{* state *}</td>       
       <td>{* actions *}</td>
    ...
    <tr class="ProductSlave list" id="ProductSlave-{$item->get('id')}"> 
        <td class="ProductSlave-count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>  
        <td >{$item->get('id')}</td> 
        <td>
            {*$item->get('linked_id')*}
            {$item->getItem()->getMaster()->get('id')}
        </td>
        {if $user->hasCredential([['superadmin','settings_product_item_mark_list']])}
         <td>
              {$item->get('mark')} 
        ...
        {/if} 
           ...
           {/if} 
            {if $user->hasCredential([['superadmin','settings_product_item_is_active_list']])}
            <td>                
                 <a href="#" title="{__('change')}" class="ProductItemSlave-Change" id="{$item->get('id')}" name="{$item->get('is_active')}"><img  src="{url('/icons/','picture')}{$item->get('is_active')}.gif" alt='{__($item->get("is_active"))}'/></a>             
            </td>  
            {/if}
            <td>
                {$item->getStatusI18n()}
            ...
            <td>               
                <a href="#" title="{__('Edit')}" class="ProductItemSlave-View" id="{$item->get('id')}">
                     <i class="fa fa-edit" style="margin-right:10px;"></i></a>    
                {if $user->hasCredential([['superadmin','settings_product_items_linked']])}
                     <a href="#" title="{__('Links')}" class="ProductItemSlave-Links" id="{$item->get('id')}">
                     <i class="fa fa-list" style="margin-right:10px;"></i></a>   
                {/if}
                {if $user->hasCredential([['superadmin','settings_product_items_linked_products']])}
                     <a href="#" title="{__('Links with product')}" class="ProductItemSlaveProduct-Links" id="{$item->get('id')}">
                     <i class="fa fa-list" style="margin-right:10px;color: green;"></i></a>   
                {/if}
                <a href="#" title="{__('Delete')}" class="ProductItemSlave-Delete" id="{$item->get('id')}" name="{$item->get('reference')}">
...
{else}
    {if $pager->getNbItems()>5}
        <input type="checkbox" id="ProductSlave-all" /> 
          <a style="opacity:0.5" class="ProductSlave-actions_items" href="#" title="{__('delete')}" id="ProductSlave-Delete">
              <img  src="{url('/icons/delete.gif','picture')}" alt='{__("Delete")}'/>
          </a>         
    {/if}
{/if}    
{include file="./_pagers/default/_default_pager_bottom.tpl" pager=$pager class="ProductSlave"} 
</div>
...
<script type="text/javascript">
	 ...
	 function changeItemState(resp) 
        {
            $.each(resp.selection?resp.selection:[resp.id], function (id) { 
                sel="a.ProductItemSlave-Change[id="+this+"]";
        ...
        function getSiteProductFilterSlaveParameters()
        {
            var params={  Product: '{$product->get('id')}', 
                        filter: {  order : { }, 
                                    search : { },
                                    equal: { 
                                         is_active : $(".ProductSlave-equal[name=is_active] option:selected").val()  
                                    },
                                lang: $("img.Product").attr('id'),                                                                                                               
                                nbitemsbypage: $("[name=ProductSlave-nbitemsbypage]").val(),
                                token:'{$formFilter->getCSRFToken()}'
                         } };
            if ($(".ProductSlave-order_active").attr("name"))
                 params.filter.order[$(".ProductSlave-order_active").attr("name")] =$(".ProductSlave-order_active").attr("id");   
            $(".ProductSlave-search").each(function() { params.filter.search[$(this).attr('name')] =$(this).val(); });            
            return params;                  
        }
        
        function updateSiteProductFilterSlave()
        {          
           return $.ajax2({ data: getSiteProductFilterSlaveParameters(), 
                            url:"{url_to('products_items_ajax',['action'=>'ListSlaveItems'])}" , 
        ...
        function updateSitePagerSlave(n)
        {
           var page_active=$(".ProductSlave-pager .ProductSlave-active").html()?parseInt($(".ProductSlave-pager .ProductSlave-active").html()):1;
           var records_by_page=$("[name=ProductSlave-nbitemsbypage]").val();
           start=(records_by_page!="*")?(page_active-1)*records_by_page+1:1;
           $(".ProductSlave-count").each(function(id) { $(this).html(start+id) }); // Update index column           
           nb_results=parseInt($("#ProductSlave-nb_results").html())-n;
           $("#ProductSlave-nb_results").html((nb_results>1?nb_results+" {__('Results')}":"{__('One result')}"));
           $("#ProductSlave-end_result").html($(".ProductSlave-count:last").html());
        }
                   
            
          {* =====================  P A G E R  A C T I O N S =============================== *}  
      
           $("#ProductSlave-init").click(function() {                 
          ...
          $('.ProductSlave-order').click(function() {
                $(".ProductSlave-order_active").attr('class','ProductSlave-order');
                $(this).attr('class','ProductSlave-order_active');
                return updateSiteProductFilterSlave();
           });
           
            $(".ProductSlave-search").keypress(function(event) {
                if (event.keyCode==13)
                    return updateSiteProductFilterSlave();
            });
            
          $("#ProductSlave-filter").click(function() { return updateSiteProductFilterSlave(); }); 
          
          $(".ProductSlave-equal[name=is_active],[name=ProductSlave-nbitemsbypage]").change(function() { return updateSiteProductFilterSlave(); }); 
          
         // $("[name=ProductSlave-name]").change(function() { return updateSiteProductFilterSlave(); }); 
           
           $(".ProductSlave-pager").click(function () {                     
                return $.ajax2({ data: getSiteProductFilterSlaveParameters(), 
                                 url:"{url_to('products_items_ajax',['action'=>'ListSlaveItems'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                 errorTarget: ".site-errors",
                                 loading: "#tab-site-dashboard-x-settings-loading",
                                 target: "#SlaveAction"
                });
         ...
         $(".ProductItemSlave-View").click( function () {                    
          ...
          $(".ProductItemSlave-Delete").click( function () { 
    ...
    $(".ProductItemSlave-Change").click(function() { 
                        return $.ajax2({ 
    ...
    $(".ProductItemSlave-Links").click( function () {                    
                return $.ajax2({ data : { ProductItem : $(this).attr('id'),
    ...
    $(".ProductItemSlaveProduct-Links").click( function () {                    
                return $.ajax2({ data : { ProductItem : $(this).attr('id'),
                                          Action: "ListPartialItemMasterSlave"
                                        },
                                loading: "#tab-site-dashboard-x-settings-loading",
