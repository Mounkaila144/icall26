...
<h3>{__("View product for mutual: %s.",$item->getMutualPartner()->get('name'))}</h3>
<div>
    <a href="#" id="MutualProduct-Save" style="display:none" class="btn"><i class="fa fa-floppy-o" style="margin-right: 10px;"></i>{__('Save')}</a>
...
<script type="text/javascript">
    ...
    $('#MutualProduct-Save').click(function(){                             
        ...
        $("input.MutualProduct[type=text]").each(function() {  params.MutualProduct[$(this).attr('name')] = $(this).val();  });         
        $("input.MutualProduct[type=radio]:checked").each(function() { params.MutualProduct[$(this).attr('name')] = $(this).val(); }); 
        return $.ajax2({ data : params,                   
                        url: "{url_to('app_mutual_ajax',['action'=>'SaveMutualProduct'])}",
                        loading: "#tab-site-dashboard-x-settings-loading",
                        errorTarget: ".site-errors",
                        target: "#actions" }); 
    });  
