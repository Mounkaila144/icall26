...
<h3>{__("New commission")}</h3>
<div>
    <a href="#" id="MutualProductCommission-Save" class="btn" style="display:none"><i class="fa fa-floppy-o" style="margin-right: 10px;"></i>{__('Save')}</a>
...
<table class="tab-form">
    ...
    <tr>
        <td><span class="label">{__("To")}</span></td>
        <td>
    ...
    <tr>
        ...
        <td>
            ...
            <input type="text" class="MutualProductCommission" name="rate" size="64" value="{if $form->hasErrors()}{$item->get('rate')}{else}{$item->getRatePercent()}{/if}"/> 
...
<script type="text/javascript">
    ...
    $('#MutualProductCommission-Cancel').click(function(){                           
        return $.ajax2({ data : { MutualProduct : "{$item->getMutualProduct()->get('id')}" },                              
                        url : "{url_to('app_mutual_ajax',['action'=>'ListMutualProductCommission'])}",
    ...
    $('#MutualProductCommission-Save').click(function(){                             
        ...
        $("input.MutualProductCommission[type=text]").each(function() {  params.MutualProductCommission[$(this).attr('name')]=$(this).val();  });  // Get foreign key                  
