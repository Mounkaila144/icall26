...
<h3>{__("New contact for mutual: %s.",$item->getPartner()->get('name'))}</h3>
<div>
    <a href="#" id="MutualPartnerContact-Save" style="display:none" class="btn"><i class="fa fa-floppy-o" style="color: #000; margin-right: 10px;"></i>{__('Save')}</a>
...
<script type="text/javascript">
    ...
    $('#MutualPartnerContact-Save').click(function(){                             
        var  params= {  MutualPartner: "{$item->getPartner()->get('id')}",      
                        MutualPartnerContact: {  
                           country : $(".MutualPartnerContact[name=country] option:selected").val() ,                                   
                           token :'{$form->getCSRFToken()}'
                        } };                
        $("input.MutualPartnerContact[type=text]").each(function() {  params.MutualPartnerContact[this.name]=$(this).val();  });  // Get foreign key                  
