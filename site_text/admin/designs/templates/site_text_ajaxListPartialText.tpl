...
<table class="tabl-list  footable table" cellpadding="0" cellspacing="0"> 
    ...
    {if $user->hasCredential([['superadmin']])}
        <th  class="footable-first-column" data-toggle="true">
            <span>{__('Module')}</span>    
            <div>
                <a href="#" class="SiteText-order{$formFilter.order.module->getValueExist('asc','_active')}" id="asc" name="module"><img  src='{url("/icons/sort_asc`$formFilter.order.module->getValueExist("asc","_h")`.gif","picture")}' alt="{__('order_asc')}"/></a>
                <a href="#" class="SiteText-order{$formFilter.order.module->getValueExist('desc','_active')}" id="desc" name="module"><img  src='{url("/icons/sort_desc`$formFilter.order.module->getValueExist("desc","_h")`.gif","picture")}' alt="{__('order_desc')}"/></a>
            </div> 
        </th>     
        {/if}
        <th data-hide="phone" style="display: table-cell;">
            <span>{__('Key')}</span>      
            <div>
                <a href="#" class="SiteText-order{$formFilter.order.key->getValueExist('asc','_active')}" id="asc" name="key"><img  src='{url("/icons/sort_asc`$formFilter.order.key->getValueExist("asc","_h")`.gif","picture")}' alt="{__('order_asc')}"/></a>
                <a href="#" class="SiteText-order{$formFilter.order.key->getValueExist('desc','_active')}" id="desc" name="key"><img  src='{url("/icons/sort_desc`$formFilter.order.key->getValueExist("desc","_h")`.gif","picture")}' alt="{__('order_desc')}"/></a>
            </div>
        </th>  
           <th data-hide="phone" style="display: table-cell;">
            <span>{__('Value')}</span>      
            <div>
                <a href="#" class="SiteText-order{$formFilter.order.value->getValueExist('asc','_active')}" id="asc" name="value"><img  src='{url("/icons/sort_asc`$formFilter.order.value->getValueExist("asc","_h")`.gif","picture")}' alt="{__('order_asc')}"/></a>
                <a href="#" class="SiteText-order{$formFilter.order.value->getValueExist('desc','_active')}" id="desc" name="value"><img  src='{url("/icons/sort_desc`$formFilter.order.value->getValueExist("desc","_h")`.gif","picture")}' alt="{__('order_desc')}"/></a>
            </div>
        </th>         
    ...
    <tr class="input-list">
     ...
     {*  <td>{* id *} {* </td> *}
      {if $user->hasCredential([['superadmin']])}
       <td>  {html_options class="SiteText-equal Select"  name="module" options=$formFilter->equal.module->getChoices()->toArray() selected=(string)$formFilter.equal.module}  </td>       
       {/if}
       <td><input type="text" class="SiteText-search" name="key"   size="5" value="{$formFilter.search.key}"/></td>
       <td><input type="text" class="SiteText-search" name="value"   size="5" value="{$formFilter.search.value}"/></td>      
       <td>{* actions *}</td>
    </tr>   
        ...
        <td class="SiteText-count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>        
         {if $user->hasCredential([['superadmin']])}
            ...
            <td>
                   ...
                   ({__($item->get('key'),[],'messages',$item->get('module'))})
            </td>            
            <td>               
...
<script type="text/javascript">
        ...
        function getSiteTextFilterParameters()
        {
            ...
            $(".SiteText-equal.Select option:selected").each(function() { params.filter.equal[$(this).parent().attr('name')] =$(this).val(); });   
            return params;                  
        }
        ...
        function updateSiteTextPager(n)
        {
          ...
          $("[name=SiteText-nbitemsbypage],.SiteText-equal").change(function() { return updateSiteTextFilter(); }); 
