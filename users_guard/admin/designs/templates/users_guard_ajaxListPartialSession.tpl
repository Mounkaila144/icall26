...
<h3>{__('Sessions')}</h3>  
  ...
  <a href="{url_to('users_guard',['action'=>'ExportCsvSessions'])}?{$formFilter->getParametersForUrl( ['equal','search'])}" class="btn UserSessions-Export"  title="{__('Sessions Export')}" >
   ...
   <a href="#" id="Session-Cancel" class="btn UserSessions-Export"  title="{__('Cancel')}" >
    ...
    <th data-hide="phone" style="display: table-cell;">#</th>
      {*  {if $pager->getNbItems()>5}<th data-hide="phone" style="display: table-cell;">&nbsp;</th>{/if}
        <th>
            ...
            <div>
                <a href="#" class="Session-order{$formFilter.order.id->getValueExist('asc','_active')}" id="asc" name="id"><img  src='{url("/icons/sort_asc`$formFilter.order.id->getValueExist("asc","_h")`.gif","picture")}' alt="{__('order_asc')}"/></a>
                <a href="#" class="Session-order{$formFilter.order.id->getValueExist('desc','_active')}" id="desc" name="id"><img  src='{url("/icons/sort_desc`$formFilter.order.id->getValueExist("desc","_h")`.gif","picture")}' alt="{__('order_desc')}"/></a>
            </div> 
        </th> *}
        </th>
