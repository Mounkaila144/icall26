...
<h3>{__('Sessions')}</h3>  
    <div>
        ...
        <a href="#" id="Session-Clean" class="btn UserSessions-Clean"  title="{__('Clean')}" >
            <i class="fa fa-cog" style="margin-right: 10px"></i>{__('Clean')}
        </a>
...
<table class="tabl-list  footable table" cellspacing="0">    
    ...
    <tr class="input-list">
        ...
        <td>{* ip *}
            {html_options class="Session-equal inputWidth" name="ip" options=$formFilter->equal.ip->getChoices()->toArray() selected=$formFilter.equal.ip}
        </td>  
        <td>{* start time *}
...
<script type="text/javascript">
    ...
    function getSiteSessionFilterParameters()
    {
        ...
        $(".Session-equal option:selected").each(function() { params.filter.search[$(this).parent().attr('name')] =$(this).val(); });            
        return params;                  
    }
    ...
    $("[name=Session-nbitemsbypage],.Session-equal").change(function() { return updateSiteSessionFilter(); }); 
    ...
    $("#Session-Cancel").click(function() {      
      ...
      $("#Session-Clean").click(function() {      
        $.ajax2({ url:"{url_to('users_dashboard_ajax',['action'=>'DashboardCleanSession'])}",
                loading: "#tab-dashboard-superadmin-loading", 
                target: "#actions"}); 
    }); 
</script>    