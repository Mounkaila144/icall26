<div id="dashboard-tabs" style="min-height: 600px">    
     <span id="progress" class="loader" style="display:none;" style="z-index: 500" ></span>
     <ul id="dashboard-tabs-ctn">
        {foreach $tabs->getSortedTabs() as $name=>$tab} 
            {if $user->hasCredential($tab->get('credentials'))}    
            <li class="site" id="site-{if $tab.target}{$tab.target}{else}{$name}{/if}" aria-controls='site-panel-{if $tab.target}{$tab.target}{else}{$name}{/if}' name="site-panel-{if $tab.target}{$tab.target}{else}{$name}{/if}">            
                <a href="{if $tab.route}{$tab->getRoute()}{else}#tab-{if $tab.target}{$tab.target}{else}{$name}{/if}{/if}" id="site-panel-{if $tab.target}{$tab.target}{else}{$name}{/if}-link"  title="{__($tab.title)}" name="tab-{if $tab.target}{$tab.target}{else}{$name}{/if}" class="tab-site tabSiteWidth">         
                     ...
                     {/if*}
                     <span class="name-tabs" data="{$tab.title}">{__($tab.title)|upper}</span>    

                     <img id="tab-site-{if $tab.target}{$tab.target}{else}{$name}{/if}-loading" class="loading" style="display:none;" style="z-index: 500" height="16px" width="16px" src="{url('/icons/loader.gif','picture')}" alt="loader"/>
                </a>              
                <span style="display:none;" id="item_tab{if $tab.target}{$tab.target|upper}{else}{$name|upper}{/if}">{__($tab.help)|default:'&nbsp;'}</span>    
            </li>
           {/if}  
        {/foreach}
        <div id="myProfile"> 
   ...
   {foreach $tabs->getComponents() as $name=>$tab}     
        {if $user->hasCredential($tab->get('credentials'))}   
            {if !$tab.route}        
                <div id="tab-{if $tab.target}{$tab.target}{else}{$name}{/if}" name="{if $tab.route}{$tab->getRoute()}{/if}">
