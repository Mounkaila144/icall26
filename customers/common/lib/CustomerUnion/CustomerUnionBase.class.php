...
class CustomerUnionBase extends mfObject2 {
     
    protected static $fields=array('name');
    const table="t_customers_union"; 
    ...
    function getUnionI18n($lang=null)
    {
        if (!$this->union_i18n)
        {
            if ($lang==null)
                $lang=mfContext::getInstance()->getUser()->getCountry();
            $this->union_i18n=new CustomerUnionI18n(array('lang'=>$lang,'union_id'=>$this->get('id')),$this->getSite());
        }    
        return $this->union_i18n;
    }    
}
