...
class app_domoprime_ajaxSendQuotationEmailForMeetingAction extends mfAction{
    ...
    public function execute(\mfWebRequest $request) {
        ...
        try 
        {           
                if (!$this->item->getMeeting()->getCustomer()->get('email'))
               ...
               // $this->getMailer()->debug(); 
                try
                {
                    ...
                    if ($this->item->hasMeeting() && $this->item->getMeeting()->hasPolluter())
                    {              
                        $polluter_quotation_model=new DomoprimePolluterQuotation($this->item->getMeeting()->getPolluter());
                    ...
                    $this->getMailer()->sendMail(                                   
                                      ...
                                      'emailQuotation',
                                      $company->get('email'),  
                                      $this->item->getMeeting()->getCustomer()->get('email'), 
                                      $model_email->get('subject') ,
                                      array("quotation"=> $this->item,'model'=>$model_email),
                                      array($pdf->getFilename())
                                    );
                                      
                                    $messages->addInfo(__(' Quotation has been sent to [{customer} - {email}].',array('customer'=>(string) $this->item->getMeeting()->getCustomer(),'email'=>$this->item->getMeeting()->getCustomer()->get('email'))));
                                  
                    $email->isSent();              
                    $response=array('info'=>__(' Quotation has been sent to [{customer} - {email}].',array('customer'=>(string) $this->item->getMeeting()->getCustomer(),'email'=>$this->item->getMeeting()->getCustomer()->get('email'))),"action"=>"SendEmail");
                ...
                // History
                $email->set('customer_id', $this->item->getMeeting()->getCustomer());               
                $email->set('email', $this->item->getMeeting()->getCustomer()->get('email'));
