...
<script type="text/javascript">
     ...
     $('#CustomerModelEmail-Cancel').click(function(){              
             return $.ajax2({ data: { filter: { lang:$("img.CustomerModelEmailI18n").attr('id'), token: "{mfForm::getToken('DomoprimeQuotationModelsFormFilter')}" } },                              
      ...
      $('#CustomerModelEmail-Save').click(function(){                             
            var  params= {         
                                DomoprimeQuotationModelI18n: { 
                                   model_i18n : { lang: "{$item_i18n->get('lang')}",model_id: "{$item_i18n->get('model_id')}"    },
                                   model : { },
                                   token :'{$form->getCSRFToken()}'
                                } };
          $("input.CustomerModelEmailI18n,textarea.CustomerModelEmailI18n").each(function() { params.DomoprimeQuotationModelI18n.model_i18n[this.name]=$(this).val(); });
          $("input.CustomerModelEmail").each(function() {  params.DomoprimeQuotationModelI18n.model[this.name]=$(this).val();  });  // Get foreign key  
        //      alert("Params="+params.toSource());   return ;          
          return $.ajax2({ data : params,                        
                           url: "{url_to('app_domoprime_ajax',['action'=>'SaveQuotationModelI18n'])}",
                           errorTarget: ".site-errors",
                           target: "#actions"}); 
        });  
