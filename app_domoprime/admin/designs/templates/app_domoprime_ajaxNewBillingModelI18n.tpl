...
<script type="text/javascript">
     ...
     $('#CustomerModelEmail-Cancel').click(function(){              
             return $.ajax2({ data: { filter: { lang:$("img.CustomerModelEmailI18n").attr('id'), token: "{mfForm::getToken('DomoprimeBillingModelsFormFilter')}" } },                              
      ...
      $('#CustomerModelEmail-Save').click(function(){                             
            var  params= {         
                                DomoprimeBillingModel: { 
                                   model_i18n : { lang: $(".CustomerModelEmailI18n[name=lang]").attr('id')  },
                                   model : { },
                                   token :'{$form->getCSRFToken()}'
                                } };
          $("input.CustomerModelEmailI18n,textarea.CustomerModelEmailI18n").each(function() { params.DomoprimeBillingModel.model_i18n[this.name]=$(this).val(); });
          $("input.CustomerModelEmail").each(function() {  params.DomoprimeBillingModel.model[this.name]=$(this).val();  });  // Get foreign key  
        //      alert("Params="+params.toSource());   return ;          
          return $.ajax2({ data : params,                        
                           url: "{url_to('app_domoprime_ajax',['action'=>'SaveNewBillingModelI18n'])}",
                           errorTarget: ".site-errors",
                           target: "#actions"}); 
        });  
