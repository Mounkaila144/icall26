...
{messages class="site-app-domoprime-results-errors"}
    ...
    <table>    
         <tr>
          ...
          <tr>
            ...
            <td>
                 {if $engine->calculationFromMeeting()}
                     {__('Meeting')}
                 {else}
                     {__('Contract')}
                 {/if}    
                ...
                <tr>
                    ...
                    <td>
                        {if $product->hasSurface()}
                            {$engine->getSurfaceFromProduct($product->getProduct())}
                        {else}
                            {__('---')}    
                        {/if}
                    </td>
    ...
    {/if}
    <div>
    Qmac: {$engine->getTotalQmac()}
    </div>
    <div>
    Qmac value: {$engine->getTotalValueQmac()} €
    </div>
    <div>
    Pose: {$engine->getTotalPose()} €
    </div>
    <div>
    Marge: {$engine->getTotalMargin()} €
    </div>
    <br>
    {/if}
    <div>
