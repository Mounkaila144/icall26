...
<script type="text/javascript">
     ...
     $('#CustomerModelEmail-Cancel').click(function(){              
      ...
      $('#CustomerModelEmail-Save').click(function(){                             
            var  params= {         
                                DomoprimePreMeetingModel: { 
                                   model_i18n : { lang: $(".CustomerModelEmailI18n[name=lang]").attr('id')  },
                                   model : { },
                                   token :'{$form->getCSRFToken()}'
                                } };
          $("input.CustomerModelEmailI18n,textarea.CustomerModelEmailI18n").each(function() { params.DomoprimePreMeetingModel.model_i18n[this.name]=$(this).val(); });
          $("input.CustomerModelEmail").each(function() {  params.DomoprimePreMeetingModel.model[this.name]=$(this).val();  });  // Get foreign key  
        //      alert("Params="+params.toSource());   return ;          
          return $.ajax2({ data : params,                        
                           url: "{url_to('app_domoprime_ajax',['action'=>'SaveNewPreMeetingModelI18n'])}",
                           errorTarget: ".site-errors",
                           target: "#actions"}); 
        });  
