...
{if $item->isLoaded()}
<h3>{__("View price for class [%s]",$item->getClass()->getI18n()->get('value'))}</h3>
<div>
...
<script type="text/javascript">
     ...
     $('#DomoprimeClassRegionPrice-Cancel').click(function(){              
             return $.ajax2({ data: { DomoprimeClass :'{$item->getClass()->get('id')}' },                              
       ...
       $('#DomoprimeClassRegionPrice-Save').click(function(){                             
            var  params= {          
                                DomoprimeRegionPriceClass: {                                    
                                   id: '{$item->get('id')}', 
                                   token :'{$form->getCSRFToken()}'
                                } };        
          $(".DomoprimeClassRegionPrice.Input").each(function() {  params.DomoprimeRegionPriceClass[$(this).attr('name')]=$(this).val();  }); 
          $(".DomoprimeClassRegionPrice.Select option:selected").each(function() {  params.DomoprimeRegionPriceClass[$(this).parent().attr('name')]=$(this).val();  }); // Get foreign key                            
          return $.ajax2({ data : params,                            
                           errorTarget: ".DomoprimeClassRegionPrice-errors",
                           loading: "#tab-site-dashboard-x-settings-loading",       
                           url: "{url_to('app_domoprime_ajax',['action'=>'SaveRegionPriceForClass'])}",
                           target: "#actions" 
                        }); 
        });  
</script>
{else}
    {__('Pricing is invalid.')}
{/if}    
