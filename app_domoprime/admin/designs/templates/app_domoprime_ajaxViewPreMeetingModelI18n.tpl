...
<table class="tab-form" cellpadding="0" cellspacing="0">
     <tr class="full-with">
      ...
      <tr class="full-with">
        <td></td>
        <td><img class="CustomerModelEmailI18n" id="{$item_i18n->get('lang')}" name="lang" src="{url("/flags/`$item_i18n->get('lang')`.png","picture")}" title="{format_country($item_i18n->get('lang'))}" />       
        </td>
    </tr>
    <tr class="full-with">
...
<script type="text/javascript">
     ...
     $('#CustomerModelEmail-Cancel').click(function(){              
             return $.ajax2({ data: { filter: { lang:$("img.CustomerModelEmailI18n").attr('id'), token: "{mfForm::getToken('DomoprimePreMeetingModelsFormFilter')}" } },                              
      ...
      $('#CustomerModelEmail-Save').click(function(){                             
            var  params= {         
                                DomoprimePreMeetingModelI18n: { 
                                   model_i18n : { lang: "{$item_i18n->get('lang')}",model_id: "{$item_i18n->get('model_id')}"    },
                                   model : { },
                                   token :'{$form->getCSRFToken()}'
                                } };
          $("input.CustomerModelEmailI18n,textarea.CustomerModelEmailI18n").each(function() { params.DomoprimePreMeetingModelI18n.model_i18n[this.name]=$(this).val(); });
          $("input.CustomerModelEmail").each(function() {  params.DomoprimePreMeetingModelI18n.model[this.name]=$(this).val();  });  // Get foreign key  
        //      alert("Params="+params.toSource());   return ;          
          return $.ajax2({ data : params,                        
                           url: "{url_to('app_domoprime_ajax',['action'=>'SavePreMeetingModelI18n'])}",
                           errorTarget: ".site-errors",
                           target: "#actions"}); 
        });  
