...
<table class="tabl-list  footable table" cellpadding="0" cellspacing="0">
    ...
    <tr class="EmailModel-list list" {if $item->hasI18n()}id="EmailModel-{$item->getI18n()->get('id')}" name="{$item->getI18n()->get('id')}"{/if}> 
        <td class="EmailModel-count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>
            {if $pager->getNbItems()>5}
                <td>        
                    {if $item->hasI18n()}
                    <input class="EmailModel-selection" type="checkbox" id="{$item->getI18n()->get('id')}" name="{$item->getI18n()->get('name')}"/>   
                    {/if}
                </td>
            {/if}          
            <td>                
            ...
            <td>               
                <a href="#" title="{__('Edit')}" class="EmailModel-View" id="{$item->get('id')}">
                     <img  src="{url('/icons/edit.gif','picture')}" alt='{__("edit")}'/></a>              
                {if $item->hasI18n()}<a href="#" title="{__('Delete')}" class="EmailModel-Delete" id="{$item->getI18n()->get('id')}"  name="{$item->getI18n()->get('value')}">
...
<script type="text/javascript">
        ...
        function getSiteDomoprimeBillingModelFilterParameters()
        {
        ...
        function updateSiteDomoprimeBillingModelFilter()
        {            
           return $.ajax2({ data: getSiteDomoprimeBillingModelFilterParameters(), 
        ...
        function updateSitePager(n)
        {
           ...
           {* ===================== L A N G U A G E =============================== *}
            ...
            $("#dialogListLanguages").bind('select',function(event){                
               ...
               updateSiteDomoprimeBillingModelFilter();
            });   
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#EmailModel-init").click(function() {   
               ...
               $.ajax2({ url:"{url_to('app_domoprime_ajax',['action'=>'ListPartialBillingModel'])}",
                         errorTarget: ".site-errors",
                         loading: "#tab-site-dashboard-x-settings-loading",                         
                         target: "#actions"}); 
           }); 
          ...
          $('.EmailModel-order').click(function() {
                ...
                return updateSiteDomoprimeBillingModelFilter();
           });
          ...
          $("#EmailModel-filter").click(function() { return updateSiteDomoprimeBillingModelFilter(); }); 
          
          $("[name=EmailModel-nbitemsbypage]").change(function() { return updateSiteDomoprimeBillingModelFilter(); }); 
         ...
         // $("[name=EmailModel-name]").change(function() { return updateSiteEmailModelFilter(); }); 
           ...
           $(".EmailModel-pager").click(function () {      
                ...
                return $.ajax2({ data: getSiteDomoprimeBillingModelFilterParameters(), 
                                 url:"{url_to('app_domoprime_ajax',['action'=>'ListPartialBillingModel'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                 errorTarget: ".site-errors",
                                 loading: "#tab-site-dashboard-x-settings-loading",
                                 target: "#actions"
                });
          ...
          $("#EmailModel-New").click( function () { 
            ...
            return $.ajax2({
                ...
                url: "{url_to('app_domoprime_ajax',['action'=>'NewBillingModelI18n'])}",
                errorTarget: ".site-errors",
                loading: "#tab-site-dashboard-x-settings-loading",
                target: "#actions"
           });
         ...
         $(".EmailModel-View").click( function () { 
                ...
                return $.ajax2({ data : { DomoprimeBillingModelI18n : { 
                                                model_id: $(this).attr('id'),
                                                lang: $("img.EmailModel[name=lang]").attr('id')                                              
                                    } },
                                loading: "#tab-site-dashboard-x-settings-loading",
                                url:"{url_to('app_domoprime_ajax',['action'=>'ViewBillingModelI18n'])}",
                                errorTarget: ".site-errors",
                                target: "#actions"});
         });                        
        ...
        $(".EmailModel-list").dblclick( function () {               
          ...
          $(".EmailModel-Delete").click( function () { 
                ...
                return $.ajax2({ data :{ DomoprimeBillingModelI18n: $(this).attr('id') },
                                 url :"{url_to('app_domoprime_ajax',['action'=>'DeleteBillingModelI18n'])}",
                                 errorTarget: ".site-errors",     
                                 ...
                                 success : function(resp) {
                                       ...
                                       {    
                                          ...
                                          {
                                              return $.ajax2({ url:"{url_to('app_domoprime_ajax',['action'=>'ListPartialBillingModel'])}",
                                                               errorTarget: ".site-errors",
