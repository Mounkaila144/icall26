...
class languages_ajaxMoveLanguageAdminAction extends mfAction {
    ...
    function execute(mfWebRequest $request) {   
        ...
        {     
            ...
            if ($form->isValid())
            {
                $item=new Language($form->getValue('id'),'admin',$site);
                $item->moveTo($form->getValue('node'));
                $item->save();
                $this->getEventDispather()->notify(new mfEvent($item, 'language.change','move')); 
                $response=array("action"=>"moveLanguageAdmin","id"=>$item->get('id'),"info"=>__("position is updated."));
            }   
            else
            {
