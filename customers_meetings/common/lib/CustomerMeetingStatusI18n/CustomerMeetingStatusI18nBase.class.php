...
class CustomerMeetingStatusI18nBase extends mfObjectI18n {
     
    protected static $fields=array('value','status_id','lang','created_at','updated_at');
    const table="t_customers_meeting_status_i18n"; 
    protected static $foreignKeys=array('status_id'=>'CustomerMeetingStatus'); // By default
      ...
      function delete()
    {
        if (parent::delete()===false)       
            return $this;
        if (!$this->hasSibbling())
            $this->getCustomerMeetingStatus()->delete();
        return $this;
    }  
   
     function getCustomerMeetingStatus()
    {
       if (!$this->_status_id)
       {
          $this->_status_id=new CustomerMeetingStatus($this->get('status_id'),$this->getSite());          
       }   
