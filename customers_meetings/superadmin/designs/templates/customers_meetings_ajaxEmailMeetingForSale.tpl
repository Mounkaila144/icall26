{messages class="`$site->getSiteID()`-customers-meeting-email-sale-`$item->getCustomer()->get('id')`-errors"}
<h3>{__("New email")|capitalize}</h3>
<div>
    <a href="#" class="btn" id="{$site->getSiteID()}-UserMeetingsEmail-Send" style="display:none"><img  src="{url('/icons/save.gif','picture')}" alt="{__('send')}"/>{__('Send')}</a>   
</div>
{if $form->isValid()}    
...
<script type="text/javascript">
    ...
    $(".UserEmail").click(function() {  $("#{$site->getSiteID()}-UserMeetingsEmail-Send").show(); });

{if $user}
    $("#{$site->getSiteID()}-UserMeetingsEmail-Send").click(function() {  
            ...
            return $.ajax2({     
                ...
                errorTarget: ".{$site->getSiteID()}-customers-meeting-email-sale-{$item->getCustomer()->get('id')}-errors",
                ...
                success: function (resp)
                         {
                             ...
                             {                                                                      
                                ...
                                {
                                    $("#{$site->getSiteID()}-UserMeetingsEmailForSale-Send").hide();
                                }
                             }    
                         }              
           });
