...
<script type="text/javascript">
    ...
    $(".CustomerMeetings-View").click(function() { 
            addSiteTabField("{$site->getSiteID()}","customers-planning",$(this).attr('id'),$(this).attr('name'));  
            return $.ajax2({ data :{ Meeting: $(this).attr('id'), target: "" },
                                 ...
                                 target: "#tab-site-panel-{$site->getSiteID()}-dashboard-site-customers-planning-"+$(this).attr('id')
                     });  
    });
    ...
    $(".CustomerMeetings-Delete").click( function () { 
                ...
                return $.ajax2({ data :{ Meeting: $(this).attr('id') },
                                 ...
                                 success : function(resp) {
                                       ...
                                       {    
                                          $("#{$site->getSiteID()}-Meeting").html("");
                                          $(".{$site->getSiteID()}-Meetings[id="+resp.id+"]").remove();
                                          count=parseInt($("#{$site->getSiteID()}-Meeting-Number").attr('name'))-1;                                         
                                          if (count == 0)
                                              text="{__("no meeting")}";
                                          else if (count == 1)
                                            text="{__("one meeting")}";
                                          ...
                                          $("#{$site->getSiteID()}-Meeting-Number").html(text);
                                          $("#{$site->getSiteID()}-Meeting-Number").attr('name',count);
                                       }       
                                 }
                     });                                        
      });
