...
class customers_meetings_ajaxSaveNewStatusLeadI18nAction extends mfAction {
    ...
    function execute(mfWebRequest $request) {             
        ...
        try
        {      
            $this->form= new CustomerMeetingStatusLeadNewForm($this->getUser()->getCountry(),$request->getPostParameter('CustomerMeetingStatusLead'),$this->site);             
            $this->status_i18n=new CustomerMeetingStatusLeadI18n(null,$this->site);
            $this->form->bind($request->getPostParameter('CustomerMeetingStatusLead'),$request->getFiles('CustomerMeetingStatusLead'));
            if ($this->form->isValid())
            {
                $this->status_i18n->getStatus()->add($this->form['status']->getValues());
                $this->status_i18n->add($this->form['status_i18n']->getValues());
                if ($this->status_i18n->getStatus()->isExist())
                    throw new mfException (__("status already exists"));   
                if ($this->form['status']->hasValue('icon'))
                {
                  $iconFile=$this->form['status']['icon']->getValue();                  
                  $this->status_i18n->getStatus()->set('icon',$iconFile->getFile()); 
                }     
                $this->status_i18n->getStatus()->save();
                if ($iconFile)
                {
                   $iconFile->save($this->status_i18n->getStatus()->getIcon()->getPath());  
                }                                                              
