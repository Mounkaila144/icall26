{messages class="customers-meeting-sms-`$item->getCustomer()->get('id')`-errors"}
<h3>{__("New SMS")|capitalize}</h3>
<div>
    <a href="#" class="btn" id="CustomerMeetingsSms-Send" style="display:none"><img  src="{url('/icons/save.gif','picture')}" alt="{__('send')}"/>{__('Send')}</a>   
</div>
{component name="/customers_communication_sms/newSms" customer=$item->getCustomer() key="customers-meeting-sms-`$item->getCustomer()->get('id')`"}
<script type="text/javascript">
    
    $(".CustomerSms").click(function() {  $("#CustomerMeetingsSms-Send").show(); });

    $("#CustomerMeetingsSms-Send").click(function() {  
            ...
            return $.ajax2({     
                ...
                errorTarget: ".customers-meeting-sms-{$item->getCustomer()->get('id')}-errors",
