...
class customers_meetings_ajaxListPartialMeetingAction extends mfAction {
    ...
    function execute(mfWebRequest $request) {        
       // $in= microtime(true);     
        ...
        $this->formFilter= new CustomerMeetingsFormFilter($this->getUser(),$request->getPostParameter('filter'));        
       // $this->getEventDispather()->notify(new mfEvent($this->formFilter, 'meeting.filter.config'));       
        $this->pager=new CustomerMeetingsPager($this->formFilter); //$this->formFilter->getObjectsForPager());  
      //  $this->time_init=microtime(true);        
        ...
        //echo "Time=".(microtime(true) - $in)."<br/>";
        
        //echo "<!-- ".($this->formFilter->equal->hasValidator('telepro_id')?"Y":"N")." -->";
