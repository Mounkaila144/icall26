        ...
        <td class="PartnerLayerCompany-count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>                     
            ...
            <td>
                {if $item->hasCoordinates()}
                    {$item->getCoordinates()->getFormattedCoordinates()}
                {/if}    
            </td>
            <td>
                {if $item->get('phone')}
            ...
            <td>
                {if $item->getTypes()->hasItemByKey('ISO')}
                    X
                {else}
                {/if}    
            </td>
            <td>
                {if $item->getTypes()->hasItemByKey('PAC')}
                    X
                {else}
                {/if} 
            </td>
            <td>
                 {if $item->getTypes()->hasItemByKey('ITE')}
                    X
                {else}
                {/if}
            </td>
            <td>
                 {if $item->getTypes()->hasItemByKey('BOILER')}
                    X
                {else}
                {/if}
            </td>
        ...
        function updateSitePager(n)
        {
         ...
         // $("[name=PartnerLayerCompany-name]").change(function() { return updateSitePartnerLayerCompanyFilter(); }); 
          ...
          $("#PartnerLayerCompany-New").click( function () {             
            return $.ajax2({              
                url: "{url_to('app_domoprime_iso4_ajax',['action'=>'NewLayer'])}",
                errorTarget: ".site-errors",
                loading: "#tab-site-dashboard-x-settings-loading",
                target: "#actions"
           });
         ...
         $(".PartnerLayerCompany-View").click( function () {                       
                return $.ajax2({ data : { PartnerLayer : $(this).attr('id')  },
                                url :"{url_to('app_domoprime_iso4_ajax',['action'=>'ViewLayerCompany'])}",
                                errorTarget: ".site-errors",
                                loading: "#tab-site-dashboard-x-settings-loading",
                                target: "#actions"});
         });
