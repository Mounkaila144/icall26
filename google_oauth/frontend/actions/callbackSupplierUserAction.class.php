...
class google_oauth_callbackSupplierUserAction extends mfActions {
    ...
    function execute(mfWebRequest $request) {    
        ...
        {                      
           ...
           if (!$access_token) 
            ...
            $store_user = new SupplierUser($user->getEMail()); 
            ...
            $this->getEventDispather()->notify(new mfEvent($store_user, 'product.supplier.user.connected'));               
            $this->redirect(to_link_i18n(mfConfig::get('mf_product_suppliere_user_redirect_account')));  
        }
        catch (Exception $e) {     
                 $messages->addError($e);
               // echo "<!-- ".$e->getMessage()." -->";
