...
class DomoprimeSimulationModelBase extends mfObject2 {
     
    protected static $fields=array('name');
    const table="t_domoprime_iso_simulation_model"; 
  ...
  /*  protected function loadByEmail($email)
     ...
     {       
         if ($this->i18n===null)
         {
             ...
             $db=mfSiteDatabase::getInstance()
                     ...
                     ->setQuery("SELECT * FROM ".DomoprimeSimulationModelI18n::getTable()." WHERE model_id='{model_id}' AND lang='{lang}';")
                     ->makeSiteSqlQuery($this->site);    
            if (!$db->getNumRows())
                return null; 
            $this->i18n=$db->fetchObject('DomoprimeSimulationModel')->loaded()->setSite($this->getSite()); 
         }   
         return $this->i18n;
     } 
    ...
    {                  
        ...
        $db=mfSiteDatabase::getInstance()
                ...
                ->setQuery("SELECT ".DomoprimeSimulationModelI18n::getFieldsAndKeyWithTable()." FROM ".  DomoprimeSimulationModelI18n::getTable().                                                 
                           " WHERE lang='{lang}';")               
                ->makeSiteSqlQuery($site); 
        if (!$db->getNumRows())
            return array();
        $items=array();
        while ($item=$db->fetchObject('DomoprimeSimulationModelI18n'))
        {
