...
{foreach $form->getProducts() as $product}    
    ...
    <tr>
        <td>
            ...
            <div>                
               <input  type="checkbox" class="DomoprimeQuotationForContract ProductItems-{$product->get('id')} Fields" name="{$product->get('id')}"  id="{$item->get('id')}" {if $form->isChecked($product,$item)}checked=""{/if}/>{$item->get('reference')}
            </div>
        {/foreach}  
        </td>
    </tr>
      ...
      $('#DomoprimeQuotationForContractView-Save').click(function(){                             
         ...
         $(".DomoprimeQuotationForContract-Products").each(function() { 
          ...
          return $.ajax2({ data : params,                          
                           url: "{url_to('app_domoprime_iso_ajax',['action'=>'SaveQuotationFromRequestForContract'])}",
                            loading: "#tab-site-dashboard-customers-contract-loading",
                           errorTarget: ".customers-contract-app-domoprime-quotation-contract-errors",
                           target: "#tab-customer-contracts-iso-quotations-{$contract->get('id')}" }); 
        }); 
