...
<table class="tabl-list  footable table" cellpadding="0" cellspacing="0"> 
    ...
    <tr class="list-header">    
        ...
        <th  class="footable-first-column" data-toggle="true">
          ...
          <th  class="footable-first-column" data-toggle="true">
            <span>{__('Total sales discount HT')}</span>               
        </th>
        {/if}
         ...
         <th  class="footable-first-column" data-toggle="true">
            <span>{__('Total discount Tax')}</span>               
        </th>
        {/if}
        {if $user->hasCredential([['superadmin','admin','app_domoprime_contract_quotation_sales_ttc']])}
         <th  class="footable-first-column" data-toggle="true">
          ...
          <th  class="footable-first-column" data-toggle="true">
            <span>{__('Total sale discount TTC')}</span>               
        </th> 
        {/if}
         {if $user->hasCredential([['superadmin','admin','app_domoprime_contract_quotation_fixed_prime']])}
         <th  class="footable-first-column" data-toggle="true">
    ...
    <tr class="DomoprimeQuotationForContract list" id="DomoprimeQuotationForContract-{$item->get('id')}"> 
        <td class="DomoprimeQuotationForContract-count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>                                              
            ...
            <td>                
             ...
             <td>                
               {$item->getFormattedTotalSaleDiscountWithoutTax()}
            </td>
            {/if}
                {if $user->hasCredential([['superadmin','admin','app_domoprime_contract_quotation_sales_taxe']])}
            <td>                
            ...
            <td>                           
                 {$item->getFormattedTotalSaleDiscountTax()}
            </td> 
            {/if}    
              {if $user->hasCredential([['superadmin','admin','app_domoprime_contract_quotation_sales_ttc']])}     
            <td>
              ...
              <td>
               {$item->getFormattedTotalSaleDiscountWithTax()}  
            </td>
            {/if}
             {if $user->hasCredential([['superadmin','admin','app_domoprime_contract_quotation_fixed_prime']])}
         <td>
              {$item->getFormatter()->getFixedPrime()->getAmount()}      
        </td>  
        {/if}
             {if $user->hasCredential([['superadmin','admin','app_domoprime_contract_quotation_prime']])}
         <td>
            ...
            <td>               
               ...
               {*  <a href="#" title="{__('Display')}" class="DomoprimeQuotationForContract-Display" id="{$item->get('id')}">
                  ...
                  {if !$item->isSigned()} 
                     ...
                     {component name="/app_domoprime_yousign_evidence/linkEvidenceIframeForQuotationPagerForContract" quotation=$item}  
                  {/if}
...
<script type="text/javascript">
        ...
        {           
           return $.ajax2({ data: getSiteDomoprimeQuotationForContractFilterParameters(), 
                            url:"{url_to('app_domoprime_iso_ajax',['action'=>'ListPartialQuotationFromRequestForContract'])}" , 
                            ...
                            target: "#tab-customer-contracts-iso-quotations-{$contract->get('id')}"
                             });
        }
        ...
        function updateSitePager(n)
        {
         ...
         // $("[name=DomoprimeQuotationForContract-name]").change(function() { return updateSiteDomoprimeQuotationForContractFilter(); }); 
           ...
           $(".DomoprimeQuotationForContract-pager").click(function () {                    
                return $.ajax2({ data: getSiteDomoprimeQuotationForContractFilterParameters(), 
                                 url:"{url_to('app_domoprime_iso_ajax',['action'=>'ListPartialQuotationFromRequestForContract'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                errorTarget: ".customers-contract-app-domoprime-iso-quotation-contract-errors",    
                                    loading: "#tab-site-dashboard-customers-contract-loading",
                                target: "#tab-customer-contracts-iso-quotations-{$contract->get('id')}"
                });
        ...
        $("#DomoprimeQuotationForContract-New").click(function() {                  
               return $.ajax2({                     
                            data : { Contract: '{$contract->get('id')}' },
                            url:"{url_to('app_domoprime_iso_ajax',['action'=>'NewQuotationFromRequestForContract'])}" , 
                            errorTarget: ".customers-contract-app-domoprime-iso-quotation-contract-errors",    
                            loading: "#tab-site-dashboard-customers-contract-loading",
                            target: "#tab-customer-contracts-iso-quotations-{$contract->get('id')}" 
                         }); 
           ...
           $(".DomoprimeQuotationForContract-View").click(function() {                  
               return $.ajax2({                     
                            ...
                            url:"{url_to('app_domoprime_iso_ajax',['action'=>'ViewQuotationFromRequestForContract'])}" , 
                            errorTarget: ".customers-contract-app-domoprime-iso-quotation-contract-errors",    
                            loading: "#tab-site-dashboard-customers-contract-loading",
                            target: "#tab-customer-contracts-iso-quotations-{$contract->get('id')}" 
                         }); 
             ...
             $(".DomoprimeQuotationForContract-Display").click(function() {                  
               return $.ajax2({                     
                            data : { Contract: '{$contract->get('id')}', DomoprimeQuotation: $(this).attr('id') },
                            url:"{url_to('app_domoprime_iso_ajax',['action'=>'DisplayQuotationForContract'])}" , 
                            errorTarget: ".customers-contract-app-domoprime-iso-quotation-contract-errors",    
                            loading: "#tab-site-dashboard-customers-contract-loading",
                            target: "#tab-customer-contracts-iso-quotations-{$contract->get('id')}" 
                         }); 
